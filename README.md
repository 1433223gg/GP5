...
- git merge 新分支名 合并某分支到当前，需要切换到要合并的项目当前分支中，再合并新分支，如果有冲突，需要先进行合并或选择，然后如果出现括号为(test|MERGING)的情况，需要先add再commit。
...
## fetch和pull的区别
1. 他们都用于从远程更新代码到本地
2. git fetch不会自动合并到当前分支(不会merge)，fetch会主动生成一个分支，
